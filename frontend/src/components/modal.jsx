...
const Modal = ({ isOpen, closeModal, titreProjet, description, pictures, urlSite, urlCode }) => {
	...
	return (
		<>
			{isOpen && (
				<div className="fixed inset-0  flex items-center justify-center bg-black bg-opacity-50 z-30 h-svh overflow-y-auto">
					<div className="flex flex-col justify-between bg-white p-5 md:p-8 max-w-screen-lg md:w-3/5 h-5/6 md:h-5/6 rounded-md ">
						...
						<div className="flex flex-col max-w-screen-lg h-4/5 md:h-5/6 ">
							...
							<p className="mt-2 md:mt-6 md:w-full h-full md:h-3/5 md:px-6 px-4 bg-[#efeff1] text-sm md:text-[16px] leading-relaxed p-scroll" dangerouslySetInnerHTML={{ __html: description }}></p>
						</div>
						...
						<div className="flex justify-end gap-2 md:gap-6">
							{urlCode && (
								<button
									className="bg-transparent text-[#2b3144] font-bold text-sm md:text-md p-2  rounded transition-all duration-700 hover:bg-[#2b3144] hover:text-[#efeff1] hover:border-[#2b3144] border border-[#2b3144] hover:border-solid"
